...
//			Point locking;
//			Point locked;
//			public LockingPair(Point locking, Point locked)
//			{
...
//		}
//		
//		LinkedList<LockingPair> lockGraph = new LinkedList<LockingPair>();
//		LinkedList<Point> movable = new LinkedList<Point>();
//		//STEP 1: build lockGraph while checking for static locks (wall-locks)
//		for (Point p : mObjects.keySet())
//		{
//			if (p != playerPosition)
//			{
//                Point upPoint = new Point(p.x,p.y-1);
//                Symbol upSym = this.get(upPoint);
//                Point downPoint = new Point(p.x,p.y+1);
//                Symbol downSym = this.get(downPoint);
//                Point leftPoint = new Point(p.x-1,p.y-1);
//                Symbol leftSym = this.get(upPoint);
//                Point rightPoint = new Point(p.x+1,p.y);
//                Symbol rightSym = this.get(rightPoint);
//                
//                //if near a wall, locked on that direction
//				boolean vertL = (upSym == Symbol.Wall || downSym == Symbol.Wall);	
//                boolean horiL = (leftSym == Symbol.Wall || rightSym == Symbol.Wall);
//                
//                // if locked by a wall on the two directions, map is locked
//                if (vertL && horiL)
//                	return true;
//                
//                //if not locked by a wall in a direction, we check dynamic locks and add edges to the graph
//                if (!vertL)
//                {
//                	if (upSym.type == Symbol.Type.Box)
//                	{
//                		lockGraph.addLast(new LockingPair(upPoint, p));
//                		vertL = true;
//                	}
//                	if (downSym.type == Symbol.Type.Box)
//                	{
//                		lockGraph.addLast(new LockingPair(downPoint, p));
//                		vertL = true;
//                	}
//                	
//                }
...
//                if (!horiL)
//                {
//                	if (leftSym.type == Symbol.Type.Box)
//                	{
//                		lockGraph.addLast(new LockingPair(leftPoint, p));
//                		vertL = true;
//                	}
//                	if (rightSym.type == Symbol.Type.Box)
//                	{
//                		lockGraph.addLast(new LockingPair(rightPoint, p));
//                		vertL = true;
//                	}
//                }
//                
//                //if the box is not locked in at least one direction, it can be the key of unlocking other boxes
//                if (!vertL || !horiL)
//                	movable.add(p);
//			}
//		}
//		
//		//STEP 2: try to unlock boxes using lockGraph and movable
//		
//	}
