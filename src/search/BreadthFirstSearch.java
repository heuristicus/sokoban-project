...
public class BreadthFirstSearch<T extends Expandable<T,U>, U> extends SearchMethod<T,U> {
    ...
    public ArrayList<U> findPath(T start, T goal) {
        ...
        if (start == null || goal == null)
            return null;
        list.add(new SearchNode<T,U>(start, null, null)); // Put the initial state onto the queue
        while(!list.isEmpty()){
            ...
            if(goal.equals(front.getNodeState())){ // Check if the popped node is the goal state
                ...
                list.addAll(front.nodeState.expand()); // Expand the nodes to generate successors, and then add to the list
            }
        }
